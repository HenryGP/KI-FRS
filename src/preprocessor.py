...
detector = Sample_Manager().faceCascade
...
def cut_image(bw_img,depth_img,depth_mtx):
   ...
   cv.imwrite("test.png",bw_img)
   #print "Faces detected: ",len(faces)
   result_bw=bw_img;result_depth=depth_img;result_mtx=depth_mtx
   for (x, y, w, h) in faces:
       result_bw = bw_img[y:y+h,x:x+w]
       result_depth = depth_img[y:y+h,x:x+w]
       result_mtx = depth_mtx[y:y+h,x:x+w]
   return result_bw, result_depth,result_mtx  
